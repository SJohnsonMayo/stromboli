...
OLD.generate_ordination <- function(data.obj, dist.obj, dist.names = c("UniFrac",
                                ...
                                cex.pt = 1.25, ellipse = T, cstar = 1, wid = 5, hei = 5, pdf = TRUE,
                                is.pvalue = FALSE, ...) {
  # Implment strata To be completed, add continuous case
  strata0 <- strata
  if (pdf) {
    if (is.null(ann)) {
      pdf(paste0("Beta_diversity_ordination_", pca.method, "_", grp.name,
                 ".pdf"), width = wid, height = hei)
    } else {
      pdf(paste0("Beta_diversity_ordination_", pca.method, "_", ann,
                 ".pdf"), width = wid, height = hei)
    }
  }
  df <- data.obj$meta.dat
  ...
  for (dist.name in dist.names) {
    ...
    xlim <- c(-max(range(abs(y[, 1]))) * 1.25, max(range(abs(y[, 1]))) *
                1.25)
    ylim <- c(-max(range(abs(y[, 2]))) * 1.25, max(range(abs(y[, 2]))) *
                1.25)
    plot(y[, 1], y[, 2], type = "n", xlim = xlim, ylim = ylim, xlab = xlab,
         ylab = ylab)
    # points(y[, 1], y[, 2], bg='yellow', col=darkcols[grp], type='p', pch
    # = pchs[grp], cex=cex.pt)
    ...
    if (!is.null(emp.lev)) {
      ...
      col1[which(levels(grp) == emp.lev)] <- rgb(1, 0, 0, 1)
      col2[which(levels(grp) != emp.lev)] <- rgb(0.5, 0.5, 0.5, 0.5)
      col2[which(levels(grp) == emp.lev)] <- rgb(1, 0, 0, 1)
    ...
    if (indiv.lab) {
      if (is.null(emp.lev)) {
        ...
        text(y[, 1], y[, 2], labels = lab.temp, cex = indiv.lab.cex,
             col = rgb(1, 0, 0, 0.75))
      } else {
        lab.temp <- rownames(dist.temp)
...
generate_PC_scatterplot <- function(data.obj, dist.obj = NULL, grp.name,
                                    adj.name = NULL, subject = NULL, dist.names = c("UniFrac", "GUniFrac",
                                                                                    "WUniFrac", "BC"), nPC = 2, pca.method = "cmd", strata = NULL,
                                    strata2 = NULL, combine.strata = FALSE, combine.strata2 = FALSE, smooth.method = "loess",
                                    pt.shape = 16, pt.alpha = 0.5, pt.size = 2, subject.pt = FALSE, ann = "",
                                    hei = NULL, wid = NULL, pdf = TRUE, gg.cmd = NULL) {
  df <- data.obj$meta.dat
  grp <- df[, grp.name]
  if (is.null(subject)) {
    ID <- NA
  ...
  if (pdf == TRUE) {
    pdf(paste0("Beta_diversity_PC_scatterplot_", ann, ".pdf"), height = hei,
        width = wid)
  }
  measures <- NULL
  x <- NULL
  for (dist.name in dist.names) {
    ...
    if (pca.method == "cmd") {
      ...
      measures <- c(measures, paste0(dist.name, " - MDS PC", 1:nPC,
                                     "(", pve, "%)"))
    }
    if (pca.method == "nmds") {
...
generate_ordination_trajectory <- function(data.obj, dist.obj = NULL, day.name = NULL,
                                           adj.name = NULL, subject = NULL, dist.names = c("UniFrac", "GUniFrac",
                                                                                           "WUniFrac", "BC"), pca.method = "cmd", strata = NULL, strata2 = NULL,
                                           combine.strata = FALSE, combine.strata2 = FALSE, smooth.method = "auto",
                                           pt.shape = 16, pt.alpha = 0.75, pt.size = 2, subject.pt = TRUE, ann = "",
                                           hei = NULL, wid = NULL, pdf = TRUE, gg.cmd = NULL) {
  nPC <- 2
  df <- data.obj$meta.dat
  ...
  if (pdf == TRUE) {
    pdf(paste0("Beta_diversity_Subject_Trajectory_", ann, ".pdf"),
  ...
  if (is.null(strata)) {
    ...
    for (measure in measures) {
      ...
      # VERY bad names with only difference in capital letters, variable
      # names are also confusing Rev: 2018_07_16
      df2 <- data.frame(xx1 = xx1, xx2 = xx2, Strata = grp2, Strata2 = grp3,
                        Strata3 = grp4, ID = ID, Day = factor(day))
      if (is.null(strata2)) {
        if (combine.strata == TRUE) {
          obj <- ggplot(df2, aes(x = xx1, y = xx2, group = ID,
                                 col = Day, linetype = Strata)) + # geom_point(size=pt.size, shape=pt.shape, alpha=pt.alpha) +
            # geom_smooth(method=smooth.method, aes(group=Strata), size=0.75) +
            labs(y = x2.measures[measure], x = x1.measures[measure],
                 title = measure)
          if (subject.pt == FALSE) {
          ...
          obj <- ggplot(df2, aes(x = xx1, y = xx2, group = ID,
                                 col = Day)) + # geom_point(size=pt.size, shape=pt.shape, alpha=pt.alpha) +
            # geom_smooth(method=smooth.method, aes(group=NA), size=0.75) +
            labs(y = x2.measures[measure], x = x1.measures[measure],
                 title = measure) + facet_grid(. ~ Strata)
          if (subject.pt == FALSE) {
        ...
        if (combine.strata == TRUE) {
          if (combine.strata2 == TRUE) {
            obj <- ggplot(df2, aes(x = xx1, y = xx2, group = ID,
                                   col = Day, linetype = Strata3)) + # geom_point(size=pt.size, shape=pt.shape, alpha=pt.alpha) +
              # geom_smooth(method=smooth.method, aes(group=Strata3), size=0.75) +
              labs(y = x2.measures[measure], x = x1.measures[measure],
                   title = measure)
            if (subject.pt == FALSE) {
              obj <- obj + geom_path(size = 0.25, alpha = 0.75,
                                     aes(col = Strata3))
            } else {
              obj <- obj + geom_path(size = 0.25, alpha = 0.75,
                                     aes(col = Strata3)) + geom_point(size = pt.size,
                                                                      shape = pt.shape, alpha = pt.alpha)
            }
          } else {
            obj <- ggplot(df2, aes(x = xx1, y = xx2, group = ID,
              ...
              # geom_smooth(method=smooth.method, aes(group=Strata), size=0.75) +
              labs(y = x2.measures[measure], x = x1.measures[measure],
                   title = measure) + facet_grid(Strata2 ~ .)
            if (subject.pt == FALSE) {
              obj <- obj + geom_path(size = 0.25, alpha = 0.75,
                                     aes(col = Strata))
            } else {
              obj <- obj + geom_path(size = 0.25, alpha = 0.75,
                                     aes(col = Strata)) + geom_point(size = pt.size,
                                                                     shape = pt.shape, alpha = pt.alpha)
            }
          }
        } else {
          ...
          obj <- ggplot(df2, aes(x = xx1, y = xx2, group = ID,
            ...
            # geom_smooth(method=smooth.method, aes(group=NA), size=0.75) +
            labs(y = x2.measures[measure], x = x1.measures[measure],
                 title = measure) + facet_grid(Strata2 ~ Strata)
          if (subject.pt == FALSE) {
            obj <- obj + geom_path(size = 0.25, alpha = 0.5, colour = "black")
          } else {
            obj <- obj + geom_path(size = 0.25, alpha = 0.5, colour = "black") +
...
# New: 2017_02_21 Separate PC plots for strata2 Rev: 2017_05_04 Add p
# value Rev: 2017_06_01 Different pages for different distances
generate_ordination_separate <- function(data.obj, dist.obj, dist.names = c("UniFrac",
                                                                            "GUniFrac", "WUniFrac", "BC"), grp.name, adj.name = NULL, emp.lev = NULL,
                                         indiv.lab = FALSE, indiv.lab.cex = 0.5, is.lightcolor = TRUE, strata = NULL,
                                         separate = NULL, layout = NULL, pc.separate, pca.method = "cmd", ann = NULL,
                                         sub = NULL, clab = 1, cex.pt = 1.25, ellipse = T, cstar = 1, wid = NULL,
                                         hei = NULL, pdf = TRUE, is.pvalue = FALSE, ...) {
  # Implment strata To be completed, add continuous case
  strata0 <- strata
  if (is.null(separate)) {
  ...
  if (pdf) {
    if (is.null(ann)) {
      pdf(paste0("Beta_diversity_ordination_", pca.method, "_", grp.name,
                 "_separate.pdf"), width = wid, height = hei)
    } else {
      pdf(paste0("Beta_diversity_ordination_", pca.method, "_", ann,
                 "_separate.pdf"), width = wid, height = hei)
    }
  ...
  for (dist.name in dist.names) {
    ...
    for (sep.level in sep.levels) {
      ...
      if (!is.null(adj.name)) {
        ...
        obj <- cmdscale(as.dist(dist.temp2), k = ncol(dist.temp2) -
                          1)
        dat2 <- apply(obj, 2, function(x) resid(lm(x ~ ., data = adj)))
      ...
      colnames(y) <- c("PC1", "PC2")
      xlim <- c(-max(range(abs(y[, 1]))) * 1.25, max(range(abs(y[,
                                                                 1]))) * 1.25)
      ylim <- c(-max(range(abs(y[, 2]))) * 1.25, max(range(abs(y[,
                                                                 2]))) * 1.25)
      plot(y[, 1], y[, 2], type = "n", xlim = xlim, ylim = ylim,
           xlab = xlab, ylab = ylab)
      # points(y[, 1], y[, 2], bg='yellow', col=darkcols[grp2], type='p', pch
      # = pchs[grp2], cex=cex.pt)
      col1 <- lightcols[1:nlevels(grp2)]
      col2 <- darkcols[1:nlevels(grp2)]
      col3 <- darkcols[grp2]
      if (!is.null(emp.lev)) {
        col1 <- col2
        col1[which(levels(grp2) != emp.lev)] <- rgb(0.5, 0.5, 0.5,
                                                    0.5)
        col1[which(levels(grp2) == emp.lev)] <- rgb(1, 0, 0, 1)
        col2[which(levels(grp2) != emp.lev)] <- rgb(0.5, 0.5, 0.5,
                                                    0.5)
        col2[which(levels(grp2) == emp.lev)] <- rgb(1, 0, 0, 1)
      ...
      title(main = sep.level, sub = paste(dist.name, "distance",
                                          pvalue))
    }
    # text(-0.25, 0.3, 'PERMANOVA p=0.016')
  }
...
generate_distance_barplot <- function(data.obj, dist.obj, dist.names = c("UniFrac",
                                                                         "GUniFrac", "WUniFrac", "BC"), grp.name, strata = NULL, within = T,
                                      between = T, bt.no = 100, ann = "") {
  strata.name <- strata
  df <- data.obj$meta.dat
  grp <- df[, grp.name]
    ...
    for (stratum in levels(strata)) {
      ...
      if (between) {
        for (grp.btw in grp.btws) {
          ind1 <- which(df2[, grp.name] == unlist(strsplit(grp.btw,
                                                           "#"))[1])
          ind2 <- which(df2[, grp.name] == unlist(strsplit(grp.btw,
                                                           "#"))[2])
          ...
          res.df <- rbind(res.df, data.frame(DistanceMetric = dist.name,
                                             Strata = stratum, Compare = "Between", DistanceType = grp.btw,
                                             Distance = mean(temp), sd = sem))
        }
      }
      if (within) {
        for (grp.wth in grp.levels) {
          ...
          res.df <- rbind(res.df, data.frame(DistanceMetric = dist.name,
                                             Strata = stratum, Compare = "Within", DistanceType = grp.wth,
                                             Distance = mean(temp), sd = sem))
        }
      }
    }
  ...
  if (between & within) {
    res.df$DistanceType <- factor(res.df$DistanceType, levels = c(grp.btws,
                                                                  grp.levels))
    levels(res.df$DistanceType) <- c(sapply(strsplit(grp.btws, "#"),
                                            paste, collapse = " vs\n"), paste("Within\n", grp.levels))
  } else {
    if (between) {
  ...
  if (is.null(strata.name)) {
    pdf(paste0("Beta_diversity_btw", between, "_wth", within, "no_strata_barplot_",
               ann, ".pdf"), width = 5, height = 5)
    ...
    for (dist.name in dist.names) {
      ...
      obj1 <- ggplot(temp, aes(x = DistanceType, y = Distance, fill = DistanceType)) +
        ...
        geom_bar(position = dodge, stat = "identity", width = 0.75,
                 colour = "black", show_guide = FALSE, size = 0.25) +
        geom_errorbar(limits, position = dodge, size = 0.25, width = 0.25) +
        labs(y = paste(dist.name, "Distance"), x = "") + theme(legend.position = "none") +
        theme(axis.text.x = element_text(angle = 90, hjust = 1))
      print(obj1)
    }
    dev.off()
  } else {
    pdf(paste0("Beta_diversity_btw", between, "_wth", within, "_strata",
               strata.name, "barplot_", ann, ".pdf"), width = 2.5 * (nlevels(strata) -
                                                                       1) + 5, height = 5)
    limits <- aes(ymax = Distance + sd, ymin = Distance - sd)
    dodge <- position_dodge(width = 0.9)
    for (dist.name in dist.names) {
      ...
      obj1 <- ggplot(temp, aes(x = Strata, y = Distance, fill = DistanceType)) +
        geom_bar(position = dodge, stat = "identity") + geom_bar(position = dodge,
                                                                 stat = "identity", colour = "black", show_guide = FALSE,
                                                                 size = 0.25) + geom_errorbar(limits, position = dodge,
                                                                                              size = 0.25, width = 0.5) + labs(y = paste(dist.name, "Distance"),
                                                                                                                               x = strata.name) + theme(axis.text.x = element_text(angle = 90,
                                                                                                                                                                                   hjust = 1))
      print(obj1)
    }
    dev.off()
...
generate_distance_boxplot <- function(data.obj, dist.obj, dist.names = c("UniFrac",
                                                                         "GUniFrac", "WUniFrac", "BC"), grp.name, strata = NULL, within = F,
                                      between = T, ann = "") {
  strata.name <- strata
  df <- data.obj$meta.dat
  grp <- df[, grp.name]
    ...
    for (stratum in levels(strata)) {
      ...
      if (between) {
        for (grp.btw in grp.btws) {
          ...
          temp <- as.vector(dist.sub[ind1, ind2])
          n <- length(temp)
          res.df <- rbind(res.df, data.frame(DistanceMetric = rep(dist.name,
                                                                  n), Strata = rep(stratum, n), Compare = rep("Between",
                                                                                                              n), DistanceType = rep(grp.btw, n), Distance = temp))
        }
      }
      if (within) {
        for (grp.wth in grp.levels) {
          ...
          n <- length(temp)
          res.df <- rbind(res.df, data.frame(DistanceMetric = rep(dist.name,
                                                                  n), Strata = rep(stratum, n), Compare = rep("Within",
                                                                                                              n), DistanceType = rep(grp.wth, n), Distance = temp))
        }
      }
    }
  }
  ...
  if (is.null(strata.name)) {
    pdf(paste0("Beta_diversity_btw", between, "_wth", within, "no_strata_boxplot_",
               ann, ".pdf"), width = 5, height = 5)
    for (dist.name in dist.names) {
      cat(dist.name, "...\n")
      temp <- res.df[res.df$DistanceMetric == dist.name, ]
      ...
      obj1 <- ggplot(temp, aes(x = DistanceType, y = Distance, col = DistanceType)) +
        geom_boxplot(position = dodge, outlier.colour = NA) + # geom_jitter(alpha=0.6, size=3.0, position = position_jitter(w = 0.1))
        # +
        labs(y = paste(dist.name, "Distance"), x = "") + theme(legend.position = "none")
      print(obj1)
    }
    ...
    pdf(paste0("Beta_diversity_btw", between, "_wth", within, "_strata",
               strata.name, "boxplot_", ann, ".pdf"), width = 2.5 * (nlevels(strata) -
                                                                       1) + 5, height = 5)
    for (dist.name in dist.names) {
      cat(dist.name, "...\n")
      temp <- res.df[res.df$DistanceMetric == dist.name, ]
...
# Rev:2016_11_25
generate_clustering <- function(data.obj, dist.obj, dist.names = c("UniFrac",
                                                                   "GUniFrac", "WUniFrac", "BC"), meta.info, cluster.method = "average",
                                is.labRow = F, cex.lab = NULL, ann = "", wid = 10, hei = 6, colFnsF = NULL,
                                colFnsC = NULL) {
  if (is.null(colFnsC)) {
    colFnsC <- c(colorRampPalette(c("black", "green")), colorRampPalette(c("black",
                                                                           "blue")), colorRampPalette(c("black", "red")))
  }
  if (is.null(colFnsF)) {
    rainbow3 <- function(x) {
      rainbow(x + 2)[1:x]
  ...
  pdf(paste0("Beta_diversity_Hierachical_clustering_", ann, ".pdf"),
  ...
  for (dist.name in dist.names) {
    ...
    for (keyID in meta.info) {
      ...
      j <- 0
      if (is.factor(x)) {
        key.list[[keyID]] <- list(breaks = levels(x), colors = (colFnsF[i +
                                                                          1][[1]])(nlevels(x)), base = NA, col.na = NA, right = F,
                                  include.lowest = F)
    ...
    dendromat(dend, mat, labRow = labRow, ylab = "Distance", main = paste(dist.name,
                                                                          "distance"), cex.lab = cex.lab)
    par(oma = c(0, 0, 0, 0))
...
OLD.perform_permanova_test <- function(data.obj, dist.obj, dist.names = c("UniFrac",
  ...
  if (!is.null(formula)) {
    ind <- apply(df[, gsub("^\\s+|\\s+$", "", strsplit(strsplit(formula,
                                                                "~")[[1]][2], "\\+")[[1]]), drop = F], 1, function(x) sum(is.na(x))) ==
      0
    df <- df[ind, ]
    if (!is.null(strata)) {
    ...
    permanovaG.obj <- NULL
    if (block.perm == F & !is.null(PermanovaG.dist)) {
      cat("\nPERMANOVA G test combining ", paste(PermanovaG.dist,
                                                 collapse = ","), "\n")
      response <- array(NA, c(sum(ind), sum(ind), length(PermanovaG.dist)),
                        dimnames = list(NULL, NULL, PermanovaG.dist))
      for (dist.name in PermanovaG.dist) {
        response[, , dist.name] <- dist.obj[[dist.name]][ind, ind]
      }
    ...
    if (pairwise == F) {
      ...
      if (block.perm == F & !is.null(PermanovaG.dist)) {
        ...
        response <- array(NA, c(sum(ind), sum(ind), length(PermanovaG.dist)),
                          dimnames = list(NULL, NULL, PermanovaG.dist))
        for (dist.name in PermanovaG.dist) {
          response[, , dist.name] <- dist.obj[[dist.name]][ind,
                                                           ind]
        }
        obj <- PermanovaG2(as.formula(paste("response", formula)),
                           df, strata = strata, ...)
      ...
      for (dist.name in dist.names) {
        ...
        for (i in 1:(grp.nlevels - 1)) {
          ...
          for (j in (i + 1):grp.nlevels) {
            ...
            ind2 <- apply(df2[, gsub("^\\s+|\\s+$", "", strsplit(strsplit(formula,
                                                                          "~")[[1]][2], "\\+")[[1]]), drop = F], 1, function(x) sum(is.na(x))) ==
              0
            df2 <- df2[ind2, ]
            dist.mat2 <- as.dist(dist.mat[ind2, ind2])
            ...
            if (block.perm == F) {
              pmat[i, j] <- pmat[j, i] <- obj$aov.tab[length(adj.name) +
                                                        1, 6]
              rmat[i, j] <- rmat[j, i] <- obj$aov.tab[length(adj.name) +
                                                        1, 5]
            } else {
              pmat[i, j] <- pmat[j, i] <- obj$aov.tab[1, 6]
              rmat[i, j] <- rmat[j, i] <- obj$aov.tab[1, 5]
            ...
            if (block.perm == F & !is.null(PermanovaG.dist)) {
              if (dist.name == dist.names[length(dist.names)]) {
                cat("\nPERMANOVA G test combining ", paste(PermanovaG.dist,
                                                           collapse = ","), "\n")
                response <- array(NA, c(sum(ind2), sum(ind2), length(PermanovaG.dist)),
                                  dimnames = list(NULL, NULL, PermanovaG.dist))
                for (dist.name in PermanovaG.dist) {
        ...
        cat("\n")
        pmat.all <- rbind(pmat.all, c(dist.name, rep("", grp.nlevels -
                                                       1)), formatC(pmat), rep("", grp.nlevels))
        rmat.all <- rbind(rmat.all, c(dist.name, rep("", grp.nlevels -
                                                       1)), formatC(rmat), rep("", grp.nlevels))
      }
      cat("\n")
      sink()
      write.csv(pmat.all, paste0("Beta_diversity_PERMANOVA_test_",
                                 ann, "_pairwiseP.csv"))
      write.csv(rmat.all, paste0("Beta_diversity_PERMANOVA_test_",
                                 ann, "_pairwiseR.csv"))
      write.csv(pmat.G, paste0("Beta_diversity_PERMANOVA_G_test_",
                               ann, "_pairwiseP.csv"))
    }
  }
...
OLD.perform_mirkat_test <- function(data.obj, dist.obj, dist.names = c("UniFrac",
  ...
  df <- data.obj$meta.dat
  if (pairwise == F) {
    ind <- apply(df[, c(grp.name, adj.name), drop = F], 1, function(x) sum(is.na(x))) ==
      0
    df <- df[ind, ]
    grp <- df[, grp.name]
    if (is.character(grp)) {
    ...
    pmat.G <- matrix(NA, grp.nlevels, grp.nlevels)
    colnames(pmat.G) <- rownames(pmat.G) <- grp.levels
    parr <- array(NA, c(grp.nlevels, grp.nlevels, length(dist.names)),
                  dimnames = list(grp.levels, grp.levels, dist.names))
    for (i in 1:(grp.nlevels - 1)) {
      grp.level1 <- grp.levels[i]
      for (j in (i + 1):grp.nlevels) {
        grp.level2 <- grp.levels[j]
        ...
        df2 <- subset(df, ind)
        df2[, grp.name] <- factor(df2[, grp.name])
        ind2 <- apply(df2[, c(grp.name, adj.name), drop = F], 1,
    ...
    write.csv(pmat.all, paste0("Beta_diversity_MiRKAT_test_", ann,
                               "_pairwiseP.csv"))
    write.csv(pmat.G, paste0("Beta_diversity_MiRKAT_O_test_", ann,
                             "_pairwiseP.csv"))
  }
}
...
perform_distance_compare_test <- function(data.obj, dist.obj, dist.names = c("UniFrac",
                                                                             "GUniFrac", "WUniFrac", "BC"), grp.name, level1, level2, level3, subject = NULL,
                                          alternative = "greater", nperm = 999, seed = 123, ann = "") {
  sink(paste0("Beta_diversity_test_", level1, "-", level2, "_", alternative,
              "_", level1, "-", level3, "_", ann, ".txt"))
  set.seed(seed)
  cat("Testing the distance ", level1, "-", level2, " is ", alternative,
      " than ", level1, "-", level3, "\n")
  df <- data.obj$meta.dat
  grp <- df[, grp.name]
  IDs <- df[, subject]
  ...
  for (dist.name in dist.names) {
    ...
    pv <- distance_compare_test(dist.obj[[dist.name]], ind1, ind2,
                                ind3, ID2, ID3, alternative, nperm)
    cat(pv, "\n")
  }
  sink()
}
